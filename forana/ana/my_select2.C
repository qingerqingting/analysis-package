void my_select2(){
  ...
  if (fight==2){
  TFile *ofile=new TFile("output_ana_001.root","RECREATE");
  }//end with if (fight==2)

  if (fight==3){
  TFile *ofile=new TFile("mctruth_inclusivemc_ana_001.root","RECREATE");
  }//end with if (fight==3)
  ...
  TH1F *h4=new TH1F ("h4","The mass spectrum of 2 gam2",60,0.4,0.7);
  h4->GetXaxis()->SetTitle("m_{2#gamma2} [GeV]");
  setymass(h4);
...
//  TLorentzVector p_gam_rho;
  ...
  TLorentzVector pip_f;
  TLorentzVector pim_f;
  TLorentzVector kp_f;
  TLorentzVector km_f;*/
  ...
  for(Long64_t j=0;j<nevent;j++)
  {
    ...
    //eta + eta'
    p_eta_etap=*gamma2_f+ *gamma3_f+ *gamma1_f+*gamma4_f+ *pip_f+ *pim_f;
