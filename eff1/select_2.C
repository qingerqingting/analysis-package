void select_2(){
  ...
  chain->Add("rightcomb_eff_m1_25.root");

  //Store the histograms.

  if (MARK==2){
    TFile *ofile=new TFile("./output_eff_m1_21.root","RECREATE");
  ...
  //Kstar
  TH1F *h9=new TH1F ("h9","The mass spectrum of #K^{+}+#pi^{-}",50,0.6,1.6);
  h9->GetXaxis()->SetTitle("m_{K^{+}+#pi^{-}} [GeV]");
  setymass(h9);
  setdivision(h9);

  TH1F *h10=new TH1F ("h10","The mass spectrum of #K^{-}+#pi^{+}",50,0.6,1.6);
...
//=***********************
    ...
    //phi + eta
    p_phi_eta=*kp_f+ *km_f+ *gamma2_f+ *gamma3_f;
    ...
    //cut,3 sigma
    //=***************************
    if (m_2gam<0.496) continue;
    if (m_2gam>0.570) continue;
