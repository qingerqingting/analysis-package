...
TFile *f =new TFile("output_data_g4pi2k2_0912.root");
...
TFile *f2 =new TFile("output_bkg_etaphietap_g4pi2k2.root");
...
RooRealVar nsig("nsig","#signal events",1,0,10000);
RooRealVar nbkg("nbkg","#background events",100,0,20000);
...
model.paramOn(frame,Layout(0.2,0.5));
...
frame->Draw();
  ...
  TLegend *leg=new TLegend(0.75,0.65,0.96,0.95);
