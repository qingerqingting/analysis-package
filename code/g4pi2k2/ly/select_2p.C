void select_2p(){
  ...
  chain->Add("rightcomb_data_g4pi2k2_0912.root");

  //Store the TreeResults.
  if (MARK==2){
    TFile *ofile=new TFile("output_data_g4pi2k2_0912.root","RECREATE");
  }//end with if (MARK==2)
  ...
  for(Long64_t j=0;j<nevent;j++)
  {
    ...
    //cuts
    if (m_2gam_1<low_eta) continue;
    if (m_2gam_1>up_eta) continue;

    if (m_2gam_2<low_eta) continue;
    if (m_2gam_2>up_eta) continue;

    if (m_2k<low_phi) continue;
    if (m_2k>up_phi) continue;
