void select_2p(){
  ...
  //    rho0->pi+ pi-
  //=*********************
  //Get the mass spectrum of eta,rho,etap,phi
  ...
  TH1F *h8=new TH1F ("h8","The mass spectrum of phi&eta",100,1,3);
  ...
  for(Long64_t j=0;j<nevent;j++)
  {
    ...
    if ((m_2pi<0.87)&&(m_2k>1.013)&&(m_2k<1.027)&&(m_gam_rho>0.939)&&(m_gam_rho<0.972)) h1->Fill(m_2gam);
    //fill rho, cut on eta, etap, phi
    if ((m_2gam>0.487)&&(m_2gam<0.576)&&(m_2k<1.027)&&(m_gam_rho>0.939)&&(m_gam_rho<0.972)) h2->Fill(m_2pi);
    //fill phi, cut on eta, rho, etap
    if ((m_2gam>0.487)&&(m_2gam<0.576)&&(m_2pi<0.87)&&(m_gam_rho>0.939)&&(m_gam_rho<0.972)) h3->Fill(m_2k);
    //fill etap, cut on eta, rho, phi
    if ((m_2gam>0.487)&&(m_2gam<0.576)&&(m_2pi<0.87)&&(m_2k>1.013)&&(m_2k<1.027)) h4->Fill(m_gam_rho);    
