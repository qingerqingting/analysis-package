void mass_window_rf_CB(){
  ...
  //Use Double CB function to fit the mass spectrum of etap.
  ...
  //The range of m_pipieta_th
  RooRealVar m_pipieta_th("m_pipieta_th","m_pipieta_th [GeV]",low,up);

  //Read the root file.
  TFile *f1=new TFile ("m_pipieta.root");
  TTree *tree =(TTree*) gDirectory->Get("TreeRes");  
  RooDataSet eta("eta","eta",tree,m_pipieta_th);
  ...
  RooCBShape sig1("sig1","sig1",m_pipieta_th,mean,sigma1,a1,n1);

  RooRealVar sigma2("sigma2","sigma2",0.05,0.001,0.1);
  RooRealVar a2("a2","a2",1,0,1000);
  RooRealVar n2("n2","n2",1,0,1000);
  //Crystal Ball function 
  RooCBShape sig2("sig2","sig2",m_pipieta_th,mean,sigma2,a2,n2);
 ...
 /* Double_t MEAN=mean.getVal();
  ...
  m_pipieta_th.setRange("cut",MEAN,up);
  RooAbsReal* int_norm_r=sig.createIntegral(m_pipieta_th,NormSet(m_pipieta_th),Range("cut"));
  Double_t area_r=int_norm_r->getVal();

  for (Double_t i=MEAN;i<up;i=i+0.001) {
  m_pipieta_th.setRange("cut",MEAN,i);
  RooAbsReal* integral_r=sig.createIntegral(m_pipieta_th,NormSet(m_pipieta_th),Range("cut"));
  ...
  m_pipieta_th.setRange("cut",low,MEAN);
  RooAbsReal* int_norm_l=sig.createIntegral(m_pipieta_th,NormSet(m_pipieta_th),Range("cut"));
  Double_t area_l=int_norm_l->getVal();

  for (Double_t i=low;i<MEAN;i=i+0.001) {
  m_pipieta_th.setRange("cut",i,MEAN);
  RooAbsReal* integral_l=sig.createIntegral(m_pipieta_th,NormSet(m_pipieta_th),Range("cut"));
  ...
  //=*********************************
  RooPlot * frame=m_pipieta_th.frame();

  //Prepare for legend.
  //The names of hist and curve are from c1.C
  //Data
  RooPlot *eta_1= eta.plotOn(frame,Binning(40));
  RooHist *h_eta=eta_1->getHist("h_eta");
  //model
  RooPlot *sig_1= sig.plotOn(frame,LineColor(kBlue));
  RooCurve *c_sig=sig_1->getCurve("sig_Norm[m_pipieta_th]");
  ...
  leg->AddEntry(h_eta,"SigMC:etap","LPE");
